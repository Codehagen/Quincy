...
- **Background compilation rather than inline**: converged across gbrain's
  nightly cycle, Letta's sleep-time, and Stanley's weekly Stories touch-up.
- **Structured data rendered to prose**: observed directly in Stanley's strategy
  page, which renders "Publish around 1 posts/day". The broken pluralisation is
  a string template with a numeric slot, which is the tell that the fields are
  the source and the paragraph is the view.
- **Hard caps on rule lists**: Stanley ships "1 / 15 rules" on Instructions. A
  cap is what stops a rule list from becoming an essay nobody obeys.
