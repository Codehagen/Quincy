...
const riff = (over: Partial<{ scrap: string; capturedAt: string; state: string }> = {}) => ({
...
describe("composeStudioGreeting", () => {
  ...
  it("names the one riff on the desk, quoted, with its day folded into the sentence", () => {
    ...
    expect(greeting.opening[1]).toContain("“I dont know, you tell me” yesterday")
  ...
  it("counts multiple riffs and quotes only the newest", () => {
    ...
    expect(greeting.opening[1]).toContain("2 riffs — the newest today: “newest scrap”")
