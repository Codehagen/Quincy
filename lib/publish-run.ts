 ...
 *
 * So a post has two hours to go out and then it has missed. Postiz allows two
 * days for the same sweep; two hours is the stricter answer because this
 * product is about a deliberate rhythm rather than throughput, and 09:00
 * writing that lands at 21:00 is not the post that was approved.
 *
 ...
 * look rather than to guess. Postiz reached the same conclusion from the other
 * direction and wrote it as `maximumAttempts: 1` on its publishing activity,
 * with the same reason in the comment: a retry of a publish whose outcome you
 * do not know is a second post.
 *
