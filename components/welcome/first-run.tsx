...
import type { InterviewState, Question } from "@/lib/onboarding"
          ...
          {inFlight ? (
            <>
              ...
              {/* Three dots would be the reflex. This says what it is doing,
                  because the last question spends a model call and the wait is
...
function Rail({ answered }: { answered: InterviewState["answered"] }) {
  return (
    ...
    <div className="overflow-hidden rounded-xl bg-card shadow-xs">
      ...
      >
        ...
        <span className="text-caption font-medium">
          Quincy knows{" "}
          <span className="font-mono tabular-nums">{answered.length}</span>{" "}
          {answered.length === 1 ? "thing" : "things"} about you
        </span>
