...
/**
 * Lanes won; Ribbon, Blocks and the round-1 layouts are deleted. The harness
 * stays so the next round is one import and one line here — same as
 * app/prototypes/rhythm.
 *
 * Worth knowing what is in git rather than lost: round 1 was Console (chosen),
 * Transcript and Stage. Stage is the shape for a later, promptier product.
 */
const VARIANTS = [{ name: "Lanes", render: () => <Lanes /> }]
...
export function Harness() {
  ...
  // Bumped to remount the variant in place, which re-runs its cut.
    ...
    <>
      <Shell>
        {/* Keyed on the variant so switching remounts and any local state
            ...
            comparison. */}
        <div key={`${current}-${replayKey}`} className="min-h-0 flex-1">
          {VARIANTS[current].render()}
        </div>
      </Shell>

      {/* The one modification PICKER.md allows: every variant puts the
          timeline along the bottom-center, which is exactly where the pill
          would sit. */}
      <nav
