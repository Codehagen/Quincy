...
import { Agenda } from "./variants/agenda"
import { Slots } from "./variants/slots"
import { Week } from "./variants/week"

/**
 * Three variants on one question: what is a publishing schedule, on screen?
 *
 * Agenda says it is a day-grouped list you scroll forward through. Week says it
 * is seven columns you plan against. Slots says it is a recurring rhythm that
 * posts fall into, which is the model lib/rhythms.ts already assumes when Week
 * Plan promises to "fill next week's slots".
 *
 * None of them is a month grid. docs/vision.md files that under what we are
 * deliberately not building: "Nobody shows lineage; everybody shows a
 * calendar."
 */
const VARIANTS = [
  { name: "Agenda", render: () => <Agenda /> },
  { name: "Week", render: () => <Week /> },
  { name: "Slots", render: () => <Slots /> },
]
...
export function Harness() {
  ...
  return (
    <>
      {/* The real shell, mirroring app/(app)/layout.tsx — a drafts page is
          judged at the width the sidebar leaves it, not at full viewport.
          No nav row highlights: usePathname reads /prototypes/drafts, and
