...
## Three decisions about shape, and why

Settled on 2026-08-08, and written down because two of them look like
implementation detail and are not.

**The catalogue is code; a row exists only once a rhythm is switched on.** The
entries live in `lib/rhythms.ts` — key, title, card description, timing,
presentation — and a fresh account loads no subscriptions at all. A catalogue
in code means a rhythm is added, renamed or removed in a pull request rather
than in a migration, which is the same argument `rhythm_id` not being a foreign
key makes.

**One manual-fire affordance per rhythm, with a run history beside it.** A
rhythm you cannot test by hand is a rhythm nobody turns on.

**A wall clock and a zone, never a cron string.** A raw `timing` of
`0 14 * * *` against a tenant pinned to `timezone: "UTC"` makes a 2:00 PM
rhythm fire at 2:00 PM UTC for everybody, so a user in Oslo gets it at 16:00.
`lib/timezone.ts`'s own header records us making precisely this mistake once
already — an 08:00 slot stored as 08:00 UTC, "right on screen, two hours wrong
in the world". See decision 1.

Considered and out of scope: a "New Ritual" button that prefills the chat
composer, so custom rhythms are authored conversationally against the same
agent. It needs a prompt store and a sandbox for what a user-written rhythm may
touch.
...
- **Custom rhythms authored in chat.** Interesting, and a
