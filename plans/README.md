...
| [025-the-channel-registry-and-connector-facts.md](025-the-channel-registry-and-connector-facts.md) | The provider registry a third channel forces (five dispatch points fail silently today), corrections owed on X and LinkedIn (X's ceiling is tier-dependent, not 280), and one fact sheet per roadmap connector from official platform docs read 2026-08-19 — Mastodon and Threads are real candidates, Bluesky forces the auth union, Instagram/YouTube/Substack are not channels, Kit is a different product shape | Written 2026-08-19 — **facts only, execution gated** on one real X post and one paying customer. Part 2 (live-channel corrections) is ungated |
| [026-shipped-work-context.md](026-shipped-work-context.md) | Why 021 shipped and produced nothing: 5 merges, 4 riffs, 12 angles, **0 drafts**, while the same merge was hand-pasted as 294 characters about a score moving. Six fixes — the brain's stories rendered in full for tool-less callers, an angle prompt that knows the material was written rather than spoken and asks for two registers, a facts paragraph plus a `forUser` sentence, the repository's own description cached for a week and carried to the writer on `riff.context`, tables and links kept because that is where the numbers are, and the backfill's missing default-branch gate | BUILT — branch `feat/shipped-work-context`, written after execution. 1053 unit tests, tsc clean. Migration written and **not applied**. Owed: one real merge through the new path and the angle comparison it exists for |
| [027-the-head-of-content-gameplan.md](027-the-head-of-content-gameplan.md) | The gameplan after measuring the field: what the live tables say is broken (0 published, every rhythm run failed, five of ten merges silent), why a merge yields a thin post (thin material, repo-facing quotes, a chat that cannot reach a PR), and four phases — richer bounded material and a brief, one question on refusal, chat tools that reach a source, an analytics table, a strategy page, rhythms that do work | IN PROGRESS — started 2026-08-27. Messaging channel (2a) deliberately held until the rest lands |
